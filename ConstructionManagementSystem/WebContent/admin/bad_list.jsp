...
<div class="title"><h2>员工不良记录</h2></div>
<div class="query">
    <div class="tabe_bot">
        <form action="${pageContext.request.contextPath}/AdminServlet?method=SearBadWork" method="post">
        <div class="l_left"><label>员工姓名：</label><input type="text" placeholder="查询该员工不良记录"  name="name" /></div>
...
<div class="table-operate ue-clear">
            <a href="${pageContext.request.contextPath}/admin/bad_add.jsp" class="add" onclick="addWork()">添加</a>
</div>
<div class="table-box">
   <table class="table" id="tbRecord">
					...
					<tbody>
					<c:forEach items="${bad}" var="p"  varStatus="status">
				      <tr class="text-c">
					    <td>${status.count}</td>
					    <td class="text-l">${p.type}</td>
					    <td>${p.date}</td>
					    <td>${p.user.name}</td>
						<td class="edit">
                            <button onclick="javascrtpt:window.location.href='${pageContext.request.contextPath}/AdminServlet?method=deleteBadTime&id=${p.id}'">删除</button>
                        </td>
                        <td></td>
						</tr>
					</c:forEach>
