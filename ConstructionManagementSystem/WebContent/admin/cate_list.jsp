...
<div class="title"><h2>材料管理</h2></div>
<div class="query">
</div>
<div class="table-operate ue-clear">
		<a href="${pageContext.request.contextPath}/admin/cate_add.jsp" class="add" onclick="addWork()">添加</a>
	</div>
<div class="table-box">
   <table class="table" id="tbRecord">
					...
					<tbody>
					<c:forEach items="${cate}" var="p"  varStatus="status">
				      <tr class="text-c">
					    <td>${status.count}</td>
					    <td class="text-l">${p.name}</td>
					    <td>${p.count}</td>
					    <td> 
					        <form action="${pageContext.request.contextPath}/AdminServlet?method=addCateNum&id=${p.id}" method="post">
                            <input type="text" placeholder="最多${p.count}"  name="num" />
                            <button type="submit">减少</button>
                            </form>
					    </td>
						<td class="edit">
						    <button onclick="javascrtpt:window.location.href='${pageContext.request.contextPath}/AdminServlet?method=deleteCate&id=${p.id}'">删除</button>
                        </td>
						</tr>
					</c:forEach>
