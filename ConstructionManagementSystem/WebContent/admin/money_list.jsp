...
<div class="title"><h2>财务记录</h2></div>
<div class="query">
    <div class="tabe_bot">
        <form action="${pageContext.request.contextPath}/AdminMoneyServlet?method=SearMoney" method="post">
        <div class="l_left"><label>近30天：</label></div>
        <input type="hidden" value="${cid}" name="cid" />
        <button class="tabe_btn " type="submit">记录查询</button>
        </form>
        <div class="clear"></div>
    </div>
</div>
<div class="table-operate ue-clear">
   当前总金额：${count}
  <c:if test="${cid == 4}">
         <a href="${pageContext.request.contextPath}/admin/money_add.jsp" class="add" onclick="addWork()">添加</a>
  </c:if>
</div>
<div class="table-box">
   <table class="table" id="tbRecord">
					...
					<tbody>
					<c:forEach items="${money}" var="p"  varStatus="status">
				      <tr class="text-c">
					    <td>${status.count}</td>
					    <td>${p.money}</td>
