...
<div class="title"><h2>所有施工计划</h2></div>
<div class="query">
    <div class="tabe_bot">
        <form action="${pageContext.request.contextPath}/AdminServlet?method=SearWork" method="post">
        <div class="l_left"><label>起始日期：</label><input id="demo" placeholder="查询开始于该日期之后的施工"  name="date" /></div>
...
<div class="table-operate ue-clear">
		<a href="${pageContext.request.contextPath}/AdminServlet?method=addWorkUI" class="add" onclick="addWork()">添加</a>
	</div>
<div class="table-box">
				      ...
				      <tr class="text-c">
					   ...
					   <td class="td-status">
						...
						<c:if test="${p.state == 2 or p.state == 3}">
							<button onclick="javascrtpt:window.location.href='${pageContext.request.contextPath}/AdminServlet?method=UpdateWork&id=${p.id}'">确认完成</button>
						</c:if>
						    <c:if test="${p.state == 4 or p.state == 1}">
                            <button onclick="javascrtpt:window.location.href='${pageContext.request.contextPath}/AdminServlet?method=deleteWork&id=${p.id}'">删除</button>
                            </c:if>
                        </td>
						</tr>
					</c:forEach>
